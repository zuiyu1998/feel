...
| 层 | 现状 | 本 feature 所需 |
|----|------|-----------------|
| `feel_entity::label` | 已有 `LabelBase`(需补充 `remark` 字段) | 新增 `UserLabel`(按 `user_id` 关联) |
| `feel_sea_orm` | 暂无 label 对应 ORM 实体 | 新增 `label`、`user_label` 表实体 |
...
| `migration` | — | 新增 `label`、`user_label` 建表迁移 |
